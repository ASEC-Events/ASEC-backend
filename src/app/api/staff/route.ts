...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const docSnap = await db.collection('staff').doc(id).get();
      if (!docSnap.exists) {
    ...
    let snapshot;
    try {
      snapshot = await db.collection('staff').orderBy('createdAt', 'desc').limit(100).get();
    } catch (orderError) {
      snapshot = await db.collection('staff').limit(100).get();
    }
